	...
	<?php
		...
		if (isset($_REQUEST['username'])){
			...
			$email = mysqli_real_escape_string($con,$email);
				...
				$query = "INSERT into `users` (username, phone, password, email)
		VALUES ('$username','$phone','".md5($password)."', '$email')";
				$result = mysqli_query($con,$query);
				if($result){
				   echo "<script type='text/javascript'> alert('Registered Successfully');location.href='parentregistration.php'</script>";
				}
			}else{
	?>
			<div class="container-fluid">
					...
					<p>Parent Name<span style="color:red">*</span></p>
